...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initApp() {
        ...
        document.getElementById('export-cocofolia-btn').addEventListener('click', () => {
            ...
            cp += `//戦闘判定\n`;
            cp += `2BB+{白兵値}%{人間性} 判定[白兵]\n`;
            cp += `2BB+{射撃値}%{人間性} 判定[射撃]\n`;
            ...
            const cc = {
                ...
                data: {
                    ...
                    params: [
                        { label: '肉体', value: String(getV('stat-body')) },
                        { label: '技術', value: String(getV('stat-tech')) },
                        { label: '感情', value: String(getV('stat-emo')) },
                        { label: '加護', value: String(getV('stat-div')) },
                        { label: '社会', value: String(getV('stat-soc')) },
                        { label: '白兵値', value: String(getV('stat-melee')) },
                        { label: '射撃値', value: String(getV('stat-ranged')) },
                        { label: '回避値', value: String(getV('stat-dodge')) },
                        { label: '行動値', value: String(getV('stat-action')) },
                    ],
                    commands: cp,
                },
            };
...
function getSheetState() {
    ...
    // 統計用に「通常時」と「魔獣化時」の両方のステータスを一時的に計算してキャプチャする
    const originalBeastMode = !!window.isBeastMode;
    ...
    // 元に戻す
    window.isBeastMode = originalBeastMode;
    calculateStats();
...
function restoreSheetState(state) {
    ...
    if (state.images) {
        ...
        if (state.images.image) {
            charData.image = state.images.image;
            const pv = document.getElementById('char-image-preview');
            const pl = document.getElementById('char-image-placeholder');
            const cl = document.getElementById('clear-image-btn');
